        ...
        <div class="relative mx-auto z-[1]">
          ...
          <?php if ($dynamic_custom == 'custom' && $choose_testimonial) : ?>
            <div id="<?php echo $testimonials_id ?>" class="testimonial-swiper relative pb-16 xl:pb-20">
              ...
              <div class="swiper-button-prev -left-4 lg:left-8 after:content-['prev'] after:text-lg after:lg:text-4xl text-brand-purple font-bold -mt-12 -translate-y-full" aria-label="Previous testimonial"></div>
              <div class="swiper-button-next -right-4 lg:right-8 after:content-['next'] after:text-lg after:lg:text-4xl text-brand-purple font-bold -mt-12 -translate-y-full" aria-label="Next testimonial"></div>
            </div>
          <?php endif; ?>
      ...
      ?>
        <div class="grid grid-cols-2 gap-4 md:grid-cols-2 md:gap-8 xl:grid-cols-4">
          <?php foreach ($checkmark_list as $check) : ?>
            <?php
            $use_image_icon = $check['use_image_icon'] ?? false;
            $image_icon = $check['image_icon'] ?? '';
            $icon = $check['icon'] ?? '';
            ...
            <div class="bg-white shadow-lg rounded p-4 md:p-8 xl:p-12 flex flex-col items-center">
              <?php if (!$use_image_icon && $icon) {
                ...
                echo '</div>';
              } else if ($use_image_icon && $image_icon) {
                echo '<div>';
                echo '<img src="' . $image_icon['url'] . '" class="block w-20 h-20 md:w-24 md:h-24 mx-auto" />';
                echo '</div>';
              } ?>
              <?php if ($text) : ?>
                <div class="text-center text-sm md:text-base xl:text-lg mt-4 lg:mt-8"><?php echo $text ?></div>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
        </div>
