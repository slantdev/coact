...
{
  echo '<article class="relative block pb-4 group">';
  echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-10 overflow-hidden rounded-lg lg:rounded-2xl" tabindex="-1" aria-hidden="true">';
  if ($image) {
...
{
  echo '<article class="relative block bg-white rounded-lg lg:rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl hover:-translate-y-2 transition duration-300">';
  echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-10 overflow-hidden rounded-t-lg lg:rounded-t-2xl" tabindex="-1" aria-hidden="true">';
  if ($image) {
...
{
  echo '<article class="relative block">';
  if ($first) {
    echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-9 lg:aspect-h-11 overflow-hidden rounded-lg lg:rounded-2xl group" aria-label="Watch video: ' . esc_attr($title) . '">';
  } else {
    echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-9 overflow-hidden rounded-lg lg:rounded-2xl group" aria-label="Watch video: ' . esc_attr($title) . '">';
  }
  if ($image) {
    echo '<img src="' . $image . '" alt="" class="object-cover w-full h-full transition duration-300 group-hover:scale-105">';
  } else {
