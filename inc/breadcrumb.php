...
function coact_yoast_seo_breadcrumb_append_link($links)
{
  ...
  $cpt_jobs_settings = get_field('cpt_jobs_settings', 'option');
  $job_parent_page = $cpt_jobs_settings['parent_page'];

  if (is_singular('job')) {
    if ($job_parent_page) {
      $permalink = get_permalink($job_parent_page->ID);
      $title = $job_parent_page->post_title;
      $breadcrumb[] = array(
        'url' => $permalink,
        'text' => $title,
      );
