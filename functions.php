...
function plain_card($link, $image, $title, $excerpt)
{
  ...
  echo '</a>';
  echo '<h4 class="post-title text-xl font-semibold my-4"><a href="' . $link . '" class="group-hover:underline">' . $title . '</a></h4>';
...
{
  echo '<div class="relative block bg-white rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl hover:-translate-y-2 transition duration-300">';
  ...
  echo '<h4 class="post-title text-xl font-semibold mb-4 text-brand-sea"><a href="' . $link . '" class="hover:underline">' . $title . '</a></h4>';
...
function featured_card($link, $image, $title, $first = false)
{
  ...
  if ($first) {
    echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-10 overflow-hidden rounded-2xl group">';
  } else {
    echo '<a href="' . $link . '" class="block aspect-w-16 aspect-h-9 overflow-hidden rounded-2xl group">';
  }
  if ($image) {
    echo '<img src="' . $image . '" alt="" class="object-cover w-full h-full transition duration-300 group-hover:scale-105">';
  } else {
  ...
  if ($first) {
    echo coact_icon(array('icon' => 'video', 'group' => 'utilities', 'size' => '128', 'class' => 'absolute w-24 h-24 top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2'));
  } else {
  ...
  echo '</a>';
  if ($first) {
    echo '<h4 class="post-title text-2xl font-semibold mt-4"><a href="' . $link . '" class="hover:underline">' . $title . '</a></h4>';
  } else {
    echo '<h4 class="post-title text-base font-semibold mt-4"><a href="' . $link . '" class="hover:underline">' . $title . '</a></h4>';
  }
  echo '</div>';
}
